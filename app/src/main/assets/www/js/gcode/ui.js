...
function onGCodeLoaded(gcode) {
      ...
      guiControllers.gcodeIndex.setValue(gr.viewModels.length - 1);
...
$(function() {
  ...
  function a() {
    //var need = '/storage/emulated/0/'+ $('.check_2',parent.document).text();
      //var need = $('.check_2',parent.document).attr('name');
      GCodeImporter.importPath('examples/part.gcode', onGCodeLoaded);
      //GCodeImporter.importPath( need, onGCodeLoaded);
      return false;
  }
  a();
  $('.gcode_examples a').on('click', function(event) {
    //console.log($('.check_2',parent.document).text());
